...
Обозначения столбцов таблицы~\ref{tab:comparison-main}: М --- масштабируемость, Н --- надёжность, П --- производительность, СВ --- сложность внедрения, СЭ --- стоимость эксплуатации, Набл. --- наблюдаемость. Графическая сводка лучших по итоговому баллу компонентов приведена на рисунке~\ref{fig:criteria-main}. Наибольший итоговый балл получил Inbox/idempotency, что отражает критичность защиты от повторного бизнес-эффекта в системах с внешними callback. Outbox и chat fan-out имеют одинаковый высокий итоговый балл, но решают разные задачи: первый связан с восстановимой публикацией событий, второй --- с массовой realtime-доставкой.

\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{../results/latest/criteria_radar.png}
\caption{Итоговые оценки ведущих архитектурных компонентов по критериям}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{../results/latest/reliability_matrix.png}
\caption{Матрица прохождения fault-injection проверок}
\label{fig:reliability-main}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{../results/latest/outbox_backlog.png}
\caption{Состояние Outbox backlog и сигнал транспортного дубля}
\label{fig:outbox-main}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{../results/latest/cache_consistency.png}
\caption{Проверка устаревшего значения при cache-aside}
\label{fig:cache-main}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{../results/latest/saga_outcomes.png}
\caption{Наблюдаемые исходы и длительность Saga}
\label{fig:saga-main}
\end{figure}
...
\url{*.png} & Графики latency, throughput, reliability, outbox backlog, cache consistency, Saga outcomes и criteria radar. \\
\bottomrule
\end{tabular}
\end{table}
